/* APTIORA Global JS v4 — theme fix, token cleanup */
'use strict';

// ── THEME ──
// Single source of truth: localStorage key = 'theme', values = 'dark' | 'light'
// applyTheme() runs immediately (before DOMContentLoaded) to prevent FOUC
(function applyThemeEarly(){
  ...
  // Will be transferred to body.light after DOM ready
})();
...
function initSidebar(){
  ...
  let path=location.pathname.split('/').pop()||location.search;
...
function initParticles(canvasId='bgCanvas'){
  ...
  function loop(){
    ...
    for(let i=0;i<pts.length;i++)for(let j=i+1;j<pts.length;j++){
      ...
      if(d<maxD){
        ctx.beginPath();
        ctx.strokeStyle=`rgba(124,58,237,${(1-d/maxD)*0.11})`;
        ctx.lineWidth=0.7;
...
function playTone(f,d,t='sine',v=0.22){
  if(!soundOn)return;
  try{
    let a=_getCtx(),o=a.createOscillator(),g=a.createGain();
...
function launchConfetti(count=60){
  for(let i=0;i<count;i++){
    ...
    c.animate(
      [{transform:'translateY(0) rotate(0)'},{transform:`translateY(${innerHeight+20}px) rotate(720deg)`}],
